...
contextBridge.exposeInMainWorld('electron', {
  ...
  getSetting: async (setting) => ipcRenderer.invoke('get-settings', setting),
  sendSettingToggle: (setting) => ipcRenderer.send('setting-toggle', setting),
  openRSFolder: (folder) => ipcRenderer.invoke('open-rs-folder', folder),
  sendSkinUpload: (type, skinData) =>
    ipcRenderer.invoke('skin-upload', type, skinData),
  ...
  sendCustomCBFilter: (matrix) =>
    ipcRenderer.invoke('custom-cb-filter', matrix),
})
